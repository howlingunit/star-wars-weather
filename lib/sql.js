...
export async function avgTemp() {
  const db = await dbConn;
  return db.all('SELECT AVG(temp) AS "avg" from data');
}

export async function amtOfRows() {
  const db = await dbConn;
  return db.all('SELECT count(*) AS "rows" from data');
}
